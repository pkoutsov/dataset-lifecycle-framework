...
```bash
cd plugins/ceph-cache-plugin/deploy/rook/setup_ceph_cluster.sh && \
./setup_ceph_cluster.sh nodename1 nodename2
```

Keep in mind that the script will uninstall any previous installations of csi-lvm and rook-ceph. If not command line arguments are passed to the script this will result in uninstalling everything.
# DLF Installation
