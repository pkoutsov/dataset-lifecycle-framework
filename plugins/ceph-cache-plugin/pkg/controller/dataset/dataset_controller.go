...
func (r *ReconcileDataset) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err == nil {
		// At this point datasetInternal exists. Lets check if this dataset
		// has ceph-caching enabled and update the DatasetInternal Status
		// accordingly if it is not already set
		if datasetInstance.Annotations["dlf-plugin-name"] == "ceph-cache-plugin" &&
			len(existingDatasetInternal.Status.Caching.Placements) == 0 {
			...
			var placements []comv1alpha1.DatasetInternalStatusCachingPlacement
			rgwPodNodeName := ""
			for _, rgwPod := range rgwPods.Items {
				rgwPodNodeName = rgwPod.Spec.NodeName
				placements = append(placements, comv1alpha1.DatasetInternalStatusCachingPlacement{
					Node:   rgwPodNodeName,
					Weight: 100,
				})
			}
			...
			err = populateListOfObjects(r.client, osdPods, []client.ListOption{
				client.InNamespace(os.Getenv("ROOK_NAMESPACE")),
			...
			for _, osdPod := range osdPods.Items {
				if rgwPodNodeName == osdPod.Spec.NodeName {
					// if osd is on the same node with
					// rgw don't change the weight
					continue
				}

				placements = append(placements, comv1alpha1.DatasetInternalStatusCachingPlacement{
					Node:   osdPod.Spec.NodeName,
					Weight: -10,
				})
			}

			existingDatasetInternal.Status.Caching.Placements = placements
